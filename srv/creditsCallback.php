...
if ($request_type == 'payments_get_items') {
  ...
  //get item info from DB table (offers)
  
  $item = array(
	'title' => "Level auto-complete",
	'description' => "Level auto-complete shows you how this level may be completed and starts next level for you.",
	'price' => 5,
	'image_url' => "https://s3.amazonaws.com/rid_swf/walkth.png",
	'item_id' => $item_id,
    'data' => $lid //name of product
  );
  ...
  if ($current_order_status == 'placed') {
    // Fulfill order based on $order_details
	$user_id = $order_details['buyer'];
	$coinsToAdd = 0;
    $level_id = 0;
    ...
    if ($earned_currency_order) {
	  ...
	  //add earned currency to user
	  $coinsToAdd = $product_amount;		
	...
	{
        $level_id = $order_details['items'][0]['data'];
		//$coinsAmountResult = mysql_query("SELECT coinsAmount FROM goods WHERE id=$itemID");
		//$coinsAmountArr = mysql_fetch_row($coinsAmountResult);
		//$coinsToAdd = $coinsAmountArr[0];
	}
    ...
    include("DB.inc.php");
    $db = DB::getInstance();
    $result = $db->query("INSERT INTO purchases (`uid`, `lid`, `date`) VALUES (" . $user_id . "," . (int)$level_id . "," . "'" . date('Y-m-d H:i:s') . "')");
